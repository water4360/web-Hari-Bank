...
SELECT * FROM (SELECT * FROM B_USER_INFO ORDER BY REG_DATETIME DESC)
  ...
  SELECT NO, TITLE, WRITER, CONTENT, TO_CHAR(REG_DATE, 'yyyy-mm-dd') regDate
    , ( SELECT COUNT(*)
        FROM T_REPLY
        WHERE BOARD_NO = B.P_NO) AS replyCnt
    FROM B_BOARD B
    ORDER BY NO DESC;
...
INSERT ALL
  INTO B_USER_INFO(USER_ID, USER_PASSWORD, KOR_NAME, BIRTHDATE, GENDER, TELECOM, PHONE_NO, EMAIL, SIGNUP_TYPE, ROLE_CODE)
                VALUES('j.hyun758', '1234', '김지현', '230613', '2', 'SKT', '01011111111', 'j.hyun123@kakao.com', 'kakao', 'H2')
  INTO B_USER_ADDRESS(USER_ID, ADDRESS_TYPE, POSTCODE, ROAD_ADDRESS, DETAIL_ADDRESS)
          VALUES('j.hyun758', '자택', '07588', '서울 강서구 공항대로43길 39', '324')
SELECT 1 FROM DUAL;
...
--휴면계좌(계좌상태) 조회
	SELECT * FROM BANK_ACCOUNT @JHBANK
	WHERE ACCOUNT_NO='1295057636' AND STATUS='0';
...
SELECT * FROM B_ACCOUNTLIST @BGHBank;
SELECT * FROM B_USER @BGHBank;

--휴면계좌(계좌상태) 조회
	SELECT * FROM B_ACCOUNT @BGHBANK BA
	JOIN B_ACCOUNTLIST @BGHBANK BAL ON BA.ACCOUNT_NO @BGHBANK = BAL.ACCOUNT_NO
	WHERE BAL.ACCOUNT_NO='1096-22476-37' AND BAL.ACCOUNTLIST_POSSIBLE='O';
...
SELECT * FROM B_ACCOUNTLIST @HJBank;
