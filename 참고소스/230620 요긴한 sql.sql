...
SELECT TO_CHAR(SYSDATE, 'YYYYMMDD') || TO_CHAR(SYSDATE, 'HH24MISS') ||'-'|| LPAD(seq_transaction_no.nextval, 3, '0') AS TRANSACTION_NO FROM dual;
...
--이체용 함수 + 거래내역B_BANK_TRANSFER에 기록 남기기 실패
--CREATE OR REPLACE PROCEDURE TRANSFER (
--  SENDER_ACCOUNT_NO IN B_USER_ACCOUNT.ACCOUNT_NO%TYPE, --보내는계좌를 B_USER_ACCOUNT테이블의.ACCOUNT_NO와 같은 형식으로 지정.
--  RECIVER_ACCOUNT_NO IN B_USER_ACCOUNT.ACCOUNT_NO%TYPE,
--  TRANSFER_AMOUNT IN NUMBER) AS --TRANSFER_AMOUNT의 파라미터타입은 NUMBER숫자
--  SENDER_BALANCE NUMBER; --보내는 계좌의 잔액 저장
--  RECEIVER_BALANCE NUMBER; --받는 계좌의 잔액 저장
--  TRANSACTION_ID VARCHAR2(20);--거래번호
--  TRANSACTION_STATUS VARCHAR2(50);--거래상태
--BEGIN
--  -- 보내는 계좌의 잔액 확인
--  SELECT TOTAL_BALANCE INTO SENDER_BALANCE --보내는 계좌의 잔액 저장
--  FROM B_USER_ACCOUNT
...
--    WHERE ACCOUNT_NO = RECIVER_ACCOUNT_NO;
--    
--    TRANSACTION_STATUS := '이체성공'; -- 거래 상태를 "Completed"로 설정
--
--  ELSE
--    TRANSACTION_STATUS := '이체실패 : 잔액부족';
--    RAISE_APPLICATION_ERROR(-20001, '(SQL)잔액이 부족합니다');
--  END IF;
--  
--  -- Transaction ID 생성 (시퀀스와 현재 날짜/시간 사용)
--  SELECT TO_CHAR(SYSDATE, 'YYYYMMDD') || '-' || TO_CHAR(SYSDATE, 'HH24MISS') || '-' || LPAD(SEQ_TRANSACTION_NO.nextval, 3, '0')
--  INTO TRANSACTION_ID FROM dual;
--
--  -- BANK_TRANSACTION 테이블에 트랜잭션 추가
--  INSERT INTO BANK_TRANSACTION (T_TRANSACTION_NO, B_BANK_CODE, ACCOUNT_NO, T_RECEIVER_ACCOUNT, T_AMOUNT, T_DATE, T_TIME, T_TO_RECEIVER, T_FROM_MEMO, T_STATUS, T_STATUS_DETAIL)
--  VALUES (TRANSACTION_ID, BANK_CODE, SENDER_ACCOUNT_NO, RECIVER_ACCOUNT_NO, TRANSFER_AMOUNT, SYSDATE, 'Transfer', TRANSACTION_STATUS); -- Assuming USD for this example
--
--  -- 충분하지 않은 잔액으로 인해 거래가 실패한 경우 에러를 발생시킴
--  IF TRANSACTION_STATUS LIKE 'Failed%' THEN
--    RAISE_APPLICATION_ERROR(-20001, TRANSACTION_STATUS);
--  END IF;
--EXCEPTION
--  WHEN OTHERS THEN
--    ROLLBACK;
...
SELECT * FROM B_TRANSACTION WHERE T_ACCOUNT_NO = '0758-53920545' OR T_RECEIVER_ACCOUNT='0758-53920545' ORDER BY T_DATE DESC, T_TIME DESC;
